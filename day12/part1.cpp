...
uint32_t countCombinationsOfSprings(const std::pair<std::string, std::vector<int>>& springsDescription, bool debug, int tabdepth)
{
        ...
        for(int offset = 0; offset < damageCount; offset++)
        {
            ...
            if(offset + 1 == damageCount)
            {
                ...
                else if(springsDescription.second.size() > 1)
                {
                    ...
                    combinations += countCombinationsOfSprings(std::make_pair(possibleRemaining, remainingDamages), debug, tabdepth + 1);
                }
...
int main(int argc, char *argv[])
{
    ...
    for(auto springLine : damagedSprings)
        answer += countCombinationsOfSprings(springLine, false, 1);
