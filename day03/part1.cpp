            ...
            if(!isdigit(character) && character != '.')
            {
                if(y > 0) // check above
                {
                    if(lines[y - 1][x] == '.') // only check both corners
                    {
                        ...
                        {
                            answer += findStringNumberInDirection(lines[y - 1], x - 1, SearchDirection::BACKWARDS);
                        }
                        if(x < line.length() - 1 && isdigit(lines[y - 1][x + 1])) // there is a number to the top right
                        {
                            answer += findStringNumberInDirection(lines[y - 1], x + 1, SearchDirection::FORWARDS);
                        }
                    }
                    else if(isdigit(lines[y - 1][x]))// there is a number across the top
                    {
                        answer += findStringNumberInDirection(lines[y - 1], x, SearchDirection::BACKWARDS);
                    }
                }

                if(x > 0 && isdigit(line[x - 1])) // check left
                {
                    answer += findStringNumberInDirection(line, x - 1, SearchDirection::BACKWARDS);
                }

                if(x < line.length() && isdigit(line[x + 1])) // check right
                {
                    answer += findStringNumberInDirection(line, x + 1, SearchDirection::FORWARDS);
                }
